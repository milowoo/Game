...
// Lock 封装分布式锁
func (s *RedisDao) Lock(key string, value interface{}, expiration time.Duration) (bool, error) {
	result, err := s.client.SetNX(s.ctx, key, value, expiration).Result()
	return result, err
}

// Unlock 释放分布式锁
func (s *RedisDao) Unlock(key string) error {
