...
*/
func (self *NatsPool) Request(subject string, v interface{}, replyPtr interface{}, timeout time.Duration) error {
	conn, err := self.Get()
	if err != nil {
		return err
	}
...
*/
func (self *NatsPool) RequestWithContext(ctx context.Context, subject string, msg interface{}, respPtr interface{}, timeout time.Duration) error {
	conn, err := self.Get()
	if err != nil {
		return err
	}
...
func (self *NatsPool) SubscribeForRequest(subject string, f RequestMsgHandler) error {
	conn, err := self.Get()
	if err != nil {
		return err
	}
