...
*/
func (self *NatsPool) Publish(subject string, msg interface{}) error {
	conn, err := self.Get()
	if err != nil {
		return err
	}
...
func (self *NatsPool) Subscribe(subject string, handler nats.MsgHandler) error {
	conn, err := self.Get()
	if err != nil {
		return err
	}
...
func (self *NatsPool) SubscribeForEncodedMsg(subject string, handler EncodedMsgHandler) error {
	conn, err := self.Get()
	if err != nil {
		return err
	}
...
*/
func (self *NatsPool) Unsubscribe(subjects ...string) error {
	conn, err := self.Get()
	if err != nil {
		return err
	}
